...
int winner(int *board){
    ...
    for(int i = 0; i < 4; i++)
        for(int j = 0; j < 4; j++)
            if(index(board, i, j + 3) == index(board, i + 1, j + 2)\
            && index(board, i + 1, j + 2) == index(board, i + 2, j + 1)\
            && index(board, i + 2, j + 1) == index(board, i + 3, j)\
            && index(board, i, j + 3) != 0)
                return index(board, i ,j);
